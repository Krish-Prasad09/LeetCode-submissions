...
//             // Pop all events that ended before the current event starts
//             while (!pq.empty() && pq.top().first < start) {
//                 maxValSoFar = max(maxValSoFar, pq.top().second);
//                 pq.pop();
//             }
            
//             // Max value is max(current best, max past valid event + current event)
//             ans = max(ans, maxValSoFar + val);
            
//             // Push current event into the heap
//             pq.push({end, val});
//         }
